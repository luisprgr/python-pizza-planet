...
def reports_ingredients(app, reports_data_ingredients):
    return [IngredientManager.create(ingredient) for ingredient in reports_data_ingredients]


@pytest.fixture
def reports_beverages(app, reports_data_beverages):
    return [BeverageManager.create(beverage) for beverage in reports_data_beverages]


@pytest.fixture
def reports_orders(app, reports_sizes, reports_ingredients, reports_beverages, reports_data_orders):
    orders = []
    for order in reports_data_orders:
        (ingredient_detail, beverage_detail) = order.pop("detail")
        ingredient_id = __search_id_by_name(ingredient_detail["ingredient"]['name'], reports_ingredients)
        beverage_id = __search_id_by_name(beverage_detail["beverage"]['name'], reports_beverages)
